...
test.describe('PR evidence DSR cockpit footer nav @pr-evidence', () => {
  test('owner opens DSR Cockpit from Legal footer and Settings Privacy', async ({
    ...
    await assertHealthyShell();
    const footerCockpitHeading = page.getByRole('heading', { name: /dsr cockpit/i });
    await expect(footerCockpitHeading).toBeVisible();
    await evidencePause(page, 600);
    await evidenceScreenshot(page, '03-dsr-cockpit-from-footer', { target: footerCockpitHeading });
    ...
    await expect(page).toHaveURL(/\/dashboard\/dsr\/?$/);
    await assertHealthyShell();
    const settingsCockpitHeading = page.getByRole('heading', { name: /dsr cockpit/i });
    await expect(settingsCockpitHeading).toBeVisible();
    await evidencePause(page, 500);
    await evidenceScreenshot(page, '05-dsr-cockpit-from-settings', { target: settingsCockpitHeading });
  });
});
