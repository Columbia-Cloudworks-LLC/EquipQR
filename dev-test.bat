...
echo Usage: dev-test.bat [critical^|full] [desktop^|mobile] [headed^|headless^|watch] [record^|video] [marketing^|debug^|debug-overlay^|support-record] [title=TEXT] [inspector] [reset-db]
exit /b 2
...
if not "%RECORDING_TITLE%"=="" (
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%scripts\run-user-regression.ps1" %PS_ARGS% -RecordingTitle "%RECORDING_TITLE%"
) else (
