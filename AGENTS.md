...
- **Main-centric git + solo velocity (#1033).** `main` is the sole production branch. Solo workflow: branch off `main` (any branch name); while working, any push should update `preview.equipqr.app` to the latest build — branch name does not matter. Ship via PR to `main` (no long-lived git `preview` integration branch). Verify locally end-to-end with zero manual steps before feature-branch push or PR open (`local-verify-before-preview-push.mdc`).
...
- **Dev stack probe false negatives.** Do not assume the local stack is down when a probe fails after an earlier successful capture — verify probe/script logic (e.g. HTTP 304 vs `fetch().ok`) with an independent check before restarting or reporting stack failure.
...
- **Preview host + Supabase (#1033):** `preview.equipqr.app` is the stable pre-production hostname for current solo work on Vercel **Preview** (not tied to a git branch named `preview`; when Preview branch tracking is enabled it follows "all unassigned branches", not a named branch picker). `preview-domain-alias.yml` aliases the latest successful Preview deployment (retired Vercel custom `staging` env). `sync-vercel-from-1password.ps1` upserts Preview vars via the **Vercel REST API** because `vercel env add` for Preview is non-interactive broken (`git_branch_required`). Cloud preview uses production Supabase `https://supabase.equipqr.app` — not persistent branch `olsdirk`; `edge-env-preview-secrets` retired from CI/workflows. Ephemeral Supabase PR branches validate schema/RLS only; do **not** wire OAuth or `VITE_SUPABASE_URL` to branch URLs — vendor callbacks stay on `https://supabase.equipqr.app/functions/v1/...`. GW/QB: test on the **local stack** before merge; cloud preview hits prod edge callbacks with Development/sandbox credentials; preview GW smoke uses Columbia Cloudworks org (`columbiacloudworks.com` in `workspace_domains`). **Vercel `main`:** Production-env build lands STAGED (`target: null`) until Promote — `equipqr.app` live only after PROMOTED, not Preview→Production graduation. GitHub repository environments removed (zero remain).
...
- Production OAuth callbacks must use `https://supabase.equipqr.app` (Google/Intuit consoles + Vercel `VITE_SUPABASE_URL`), but edge auto-injects `SUPABASE_URL` as `https://ymxkzronkhwxzcdcbnwq.supabase.co`. Code maps the project URL to the custom domain; keep `GW_OAUTH_REDIRECT_BASE_URL=https://supabase.equipqr.app` on `edge-env-prod-secrets` and sync via `.\scripts\sync-supabase-secrets-from-1password.ps1 -OpItem edge-env-prod-secrets` until drift checks pass without it.
...
- PR evidence upload reads `SUPABASE_URL` from `app-env-preview-public` and `prod_service_role_key` from `supabase-write` (not `VITE_SUPABASE_URL`, `preview_service_role_key`, or `edge-env-preview-secrets`). Demo MP4 upload reads **`GH_SESSION_TOKEN`** (User-scope GitHub `user_session` cookie, not PAT) via `scripts/upload-github-asset.ts`; one-time setup in `scripts/pr-evidence/README.md`. Desktop recordings use `scripts/lib/recording-quality.mjs` (1920×1080 Playwright video → H.264 MP4); mobile captures use `-MobileViewport` (390×844). Playwright `recordVideo.size` must match the viewport.
