...
Today that comparison still uses `edge-env-prod-secrets` because live preview is
production-backed. Once the cutover in `docs/ops/preview-persistent-branch.md`
lands, change the comparison target to `edge-env-preview-secrets`.
...
- **Docs site, PWA, and public media:** `equipqr.info` is a standalone VitePress site in `docs/` (`docs:dev` on 5174, `docs:build`, `docs:preview`); app links resolve through `documentationUrl.ts`. **`docs:build`** runs `externalize-docs-inline-scripts.mjs` so VitePress inline bootstrap scripts become content-addressed `/assets/inline.*.js` files — committed `docs/vercel.json` keeps static `script-src 'self'` (Vercel serves committed headers, so post-build sha256 hashes from older flows drifted and broke hydration/clicks on #1147/#1158). Marketing stills live in git at `public/images/landing/` — `landingImage()` returns same-origin `/images/landing/{key}` from closed `LANDING_IMAGE_KEYS` (rejects `pr-evidence/` and `..`; no `VITE_SUPABASE_URL`), so a local Force reset does not empty them. Homepage collage tiles are generated from `public/images/marketing/` sources. Videos stay on production Storage via `landingVideo()` → `https://supabase.equipqr.app/storage/v1/object/public/landing-page-videos/` (`resolvePublicSupabaseOrigin` pins that host when local `VITE_SUPABASE_URL` differs; do not vendor mp4/webm). App image buckets stay private with signed URLs except organization logos. PR evidence and docs-media stay on Supabase, not in marketing helpers. PWA service worker is disabled on Vite dev (`localhost:8080`); SW/offline-shell verification requires `npm run build` + `vite preview`, and stale chunk 404s recover through chunk-load reload handling. **docs-media publishing:** upload curated captures via `npx tsx dev/upload-screenshot.ts <local> <storage-path> docs-media` with env from `Set-PrEvidenceUploadEnvironment` (`PrEvidenceCommon.ps1`); docs embed images as `https://supabase.equipqr.app/storage/v1/object/public/docs-media/...` and videos as bare URLs in angle brackets. **Public legal:** `/right-to-repair` (Legal footer; stance, not a contract) — EquipQR supports independent repair and will not hold operational records hostage; keep with Terms/Privacy/Security under `src/pages`.
