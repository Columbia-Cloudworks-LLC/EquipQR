...
- **Equipment serial & offline:** Duplicate org serials warn with link to existing equipment but do not block create (unique constraint on `organization_id, serial_number` removed). Offline creates queue in localStorage; persistent 409 sync failures usually mean the record already exists, not offline mode.
- **Dashboard team scope & public QR:** Dashboard stat widgets and `get_dashboard_trends` respect TopBar `useSelectedTeam` (including unassigned-only). Public `/qr/equipment/:id` uses `useSimpleOrganizationSafe()` — not `useOrganization()` outside `SimpleOrganizationProvider`.
