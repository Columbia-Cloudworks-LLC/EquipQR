...
- **Stay local until asked to publish.** Default is local-iterate on the current checkout (`branching.mdc`). Do not create a branch, spawn a worktree, open a PR, or merge for a design tweak or other unpublished work. When the user does ask to publish, feature-branch PRs into **`preview`** follow open → evidence published → CI green → Supabase green or skipped → **merge** (`pr-merge-ready-workflow.mdc`). **Do not wait for Qodo** — it is retired. Review threads are not a merge blocker unless the user asks to address them. Do not open a PR and walk away. A linked Cursor worktree is not publish authorization. Invoking `/itil-issue-resolver` with an issue to land authorizes the integrate-and-merge loop; a small local change without that ask does not.
