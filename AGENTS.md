...
- **`/itil-issue-resolver` always means merge-ready PR.** Invoking the resolver skill is authorization for the full integrate loop in `itil-issue-resolver/SKILL.md` §6 — not implementation-only or open-and-walk-away. Babysit until `pr-merge-ready-workflow.mdc` exit criteria pass.
