...
- Google OAuth Verification Center expects live `https://equipqr.app/privacy-policy` (not `/privacy`, which serves the empty SPA shell). `@equipqr.app` does not forward email — GCP developer contact must be a reachable inbox.
- PWA service worker is disabled on Vite dev (`localhost:8080`); SW and offline-shell verification require `npm run build` + `vite preview` (port 4173 / `E2E_PWA_PREVIEW_URL` pattern).
