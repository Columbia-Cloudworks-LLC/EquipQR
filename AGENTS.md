...
- Preview and local QuickBooks use Intuit **Development** keys (same `INTUIT_CLIENT_ID` as `edge-env-local-dev`) plus `QBO_USE_SANDBOX=true` on edge; production (`main`) uses Production keys without the sandbox flag. Intuit Developer Portal OAuth redirect URIs are **per app tier** — preview Supabase callback belongs only on the Development app, not Production. Reconnect QB on preview after switching key tiers or tokens. `VITE_INTUIT_CLIENT_ID` (`app-env-preview-public`) must match `INTUIT_CLIENT_ID` (`edge-env-preview-secrets`).
