...
- **Product onboarding:** new org owners/admins redirect to `/dashboard/onboarding/getting-started` until `organization_members.product_onboarding_completed_at` is set via `complete_product_onboarding`. Wizard steps: create first team (internal department or customer account), create first equipment with team assignment, QR printing guidance (copy URL or download PNG/JPG). Equipment `team_id` links QuickBooks customer invoicing.
