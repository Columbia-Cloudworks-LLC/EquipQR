...
- **Zero npm install debt.** Address deprecation warnings and `npm audit` findings immediately — do not defer; target `npm audit` → 0. Fresh clone bootstrap is **`npm run ci:install`** or **`npm-ci-safe.bat`** on Windows (not bare `npm ci` while Vite/Vitest may hold native binary locks); Linux/CI still uses `npm ci --prefer-offline --no-audit`. Reload Cursor after install so the Vitest extension finds `node_modules`.
