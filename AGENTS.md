...
- **Main-centric git + solo velocity (#1033).** Solo workflow: one feature branch at a time → PR → `main`; git `preview` is legacy/retiring — do not branch off or open PRs into git `preview`. Verify locally end-to-end with zero manual steps before feature-branch push or PR open (`local-verify-before-preview-push.mdc`).
