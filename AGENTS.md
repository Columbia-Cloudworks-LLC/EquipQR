...
- Google Workspace OAuth must explicitly request `openid`, `email`, and `profile` (revoked grants break userinfo); callback error redirects must validate via `validate_google_workspace_oauth_session` before restoring context; URL handlers must clear `gw_connected` when processing `gw_error`.
...
- Work order reassignment preserves workflow status via `resolveStatusAfterAssignment` (e.g. `in_progress` stays `in_progress`; only unassigned-from-active statuses revert toward `submitted`).
- Work order details reads `workOrderKeys.detail` (`useWorkOrderById`); mutation hooks must call `invalidateWorkOrderCaches` in `invalidateWorkOrderQueries.ts`, not list-only invalidation.
