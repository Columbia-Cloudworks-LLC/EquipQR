...
description: Mandates an ITIL-style Change Record as the only acceptable planning artifact before code in EquipQR, produced exclusively in Plan mode and written so the cheapest capable model can implement it. The Change Record is the user's "authorization to build" — the agent generates it, posts it as a comment on the linked GitHub issue (BEFORE the user clicks "build it" so the change authority reviews it on the audit-trail thread, not just in Cursor's plan panel), waits for explicit approval, and only then begins implementation. Implementation Steps must be cheap-model executable; if the work is too broad for one Composer 2 / Auto pass, split it into smaller independently verifiable Change Records. After implementation, issue-tied work uses a branch off origin/preview, push, and (in linked worktrees per branching.mdc rule 8) a PR into preview; the issue is treated as fixed only once the change is integrated into preview (PR merged, or authorized direct push to preview on the main-worktree fast path). Implementation Steps always end with a mandatory final step that posts a follow-up on the issue with commit SHA(s), PR link when a PR was opened, and closure semantics tied to merge into preview. Ideally seeded by a prior itil-problem-record or itil-service-request, but may be invoked standalone for ad-hoc features. Includes External Setup Procedures — researched click-by-click steps for every vendor dashboard action the change requires, with "could not confirm" callouts when a feature can't be verified live. Use whenever creating or presenting an implementation plan, when the user says "draft the change record", "plan this", "scope this", or after a Problem Record or Service Request has been posted and the user is ready to authorize the fix.
---
...
6. **Implementation Steps MUST end with a final step that posts a follow-up comment on the linked GitHub issue after `git push` (and after opening a PR into `preview` when required by [branching rule](../../rules/branching.mdc) rule 8).** The follow-up MUST include the commit SHA(s); when a PR was opened, a markdown link to that PR; and explicit wording that the issue is **fixed only once the change is integrated into `preview`** (PR merged into `preview`, or the authorized main-worktree direct-push path to `preview` per rule 8 — not merely when commits exist on a feature branch). This closes the audit loop: upstream Service Request / Problem Record → Change Record → push/PR → follow-up tying shipped commits to authorization. The follow-up MUST link back to the original Change Record comment URL (captured per Rule 5). Skip ONLY when the change is ad-hoc with no GitHub issue.
...
2. […]
3. […]
N. **(Mandatory final step, when this Change Record is tied to a GitHub issue)** After `git push` succeeds — and after opening a PR into `preview` with `gh pr create --base preview` when [branching rule](../../rules/branching.mdc) rule 8 requires it (linked worktree under `~/.cursor/worktrees/EquipQR/`); skip opening a PR only on the authorized main-worktree fast path (`C:\Users\viral\EquipQR` on `preview`, direct push to `preview`) — post a follow-up comment on the linked issue (the same `#<number>` referenced in **Short Description**) using `gh issue comment <number> --repo <owner>/<repo> --body-file <temp-file>`. The follow-up body MUST contain: (a) a one-line status summary like `Implemented in commit <sha-short> on branch <branch-name>`, (b) a markdown link back to the Change Record comment URL captured at plan-presentation time per Mandatory rule 5 (e.g. `Change Record: <url>`), (c) when a PR was opened, a markdown link to that PR and a line stating the issue is **resolved when that PR merges into `preview`** (if the PR is not yet merged, say it is pending merge), (d) when the main-worktree fast path was used, state that the fix is on `preview` via direct push per branching rule 8, (e) one line per AI Verification command that ran with its pass/fail outcome, (f) any deviations from the original plan (steps that proved wrong mid-flight and were amended) with a one-line rationale per deviation. Skip this final step ONLY when the Change Record is ad-hoc with no GitHub issue.
...
- **Commits:** Conventional Commits. If tied to an issue, body includes `Resolves #<number>` (or the PR body should reference the issue so merge into `preview` closes it when appropriate).
- **PR:** For issue-tied work, open a PR into `preview` after push when [branching rule](../../rules/branching.mdc) rule 8 requires it (linked worktrees under `~/.cursor/worktrees/EquipQR/` → `gh pr create --base preview --head <branch>`). On the main worktree at `C:\Users\viral\EquipQR` on `preview`, use the authorized direct-push fast path to `preview` instead — no PR unless the user asks. Ad-hoc (no issue): same worktree rule — PR in linked worktrees when integrating via a feature branch; otherwise follow user direction.
- **Issue fixed:** Only after the change is **integrated into `preview`** — PR merged into `preview`, or commits on `preview` via the authorized main-worktree path — not merely when a feature branch exists or a push completes to that branch.
...
Reply "approved" / "go" / "build it" (or click the build action) to begin execution. The final Implementation Step will post a follow-up on the same issue with commit SHA, PR link when applicable, and closure tied to **merge into `preview`** (per [branching rule](../../rules/branching.mdc) rule 8), closing the audit loop.
```
...
- **Branch & Commit Plan**: Must respect the [branching rule](../../rules/branching.mdc) — branch off `origin/preview`, integrate via PR into `preview` when rule 8 requires it (linked worktrees), or via authorized direct push to `preview` on the main-worktree fast path; never target `main` unless the user said "hotfix".
...
4. Execute **Implementation Steps** in order, exactly as written, **except** defer the **last** step (the mandatory issue follow-up per Mandatory rule 6) until after steps 7–8 complete — that follow-up must run only after `git push` and after opening a PR when step 8 requires it. All other steps: no scope creep, no opportunistic refactors. If a step proves wrong mid-flight, **STOP**, report it, amend the Change Record (and edit the GitHub comment in place per the Plan-mode revise rule above), then continue.
...
7. Commit with Conventional Commits referencing the issue (`Resolves #<number>`) when applicable. Push the branch (or push to `preview` on the authorized main-worktree fast path per [branching rule](../../rules/branching.mdc) rule 8).
8. **Integrate per rule 8:** If you are in a linked worktree under `~/.cursor/worktrees/EquipQR/`, open a PR into `preview` with `gh pr create --base preview --head <branch> --title "<conventional-commit title>"` (use `--body-file` for multi-line bodies on PowerShell). Capture the PR URL. If you are on the main worktree at `C:\Users\viral\EquipQR` on `preview` with authorization to push directly, skip opening a PR and ensure commits are on `preview` via `git push origin preview`. Ad-hoc changes with no issue: open a PR when rule 8 requires it; otherwise follow user direction.
9. **Execute the mandatory final Implementation Step (per Mandatory rule 6)**: post a follow-up comment on the linked issue using the format defined in **Authoring constraints → Implementation follow-up comment format**. Capture the commit SHA from `git rev-parse HEAD` (or from `git log -1 --format=%H`), the short SHA from `git rev-parse --short HEAD`, the branch name, the PR URL when a PR was opened, the AI Verification outcomes from your earlier execution, and any deviations from the original plan. Write the body to a temp file, post via `gh issue comment <number> --repo <owner>/<repo> --body-file <temp-file>`, capture the comment URL, log it in chat, and delete the temp file. **This step is mandatory whenever the Change Record is tied to a GitHub issue and is the final action of the entire flow** — without it, the audit loop is open.
...
- For PR readiness after the branch is pushed and (when required) the PR is opened into `preview`, follow [raise](../raise/SKILL.md).
