...
1. **(Issue-tied — mandatory first execution step and first `CreatePlan` todo)** Post the Change Record on the linked GitHub issue before any branch work, infrastructure steps, or product code edits. **Preferred:** `.\scripts\itil\Publish-ItilArtifact.ps1 -Issue <number> -ArtifactType ChangeRecord -BodyFile <temp-body.md> -Json` — capture `commentUrl` and log it in chat. **Fallback:** `gh issue comment <number> --repo <owner>/<repo> --body-file <temp-file>`. If the Change Record was **already** posted in Plan mode (Mandatory rule 5) before approval, treat this step as satisfied: confirm the existing `commentUrl` in chat; do **not** post a duplicate thread. **Ad-hoc (no issue):** write `N/A — no GitHub issue; skip posting.`
...
6. Call `CreatePlan` with the same Change Record markdown so Cursor surfaces the approve/build affordance. When the tool also emits a **todo** list, the **first** todo must be "Post the Change Record on the linked GitHub issue (capture `commentUrl`)." and the **second** must be "Create a new branch from `origin/preview` (`git fetch` + `git switch -c <branch> origin/preview`)." If the comment was already posted in steps 1–5 above, mark the first todo **completed** immediately and paste the known URL.
