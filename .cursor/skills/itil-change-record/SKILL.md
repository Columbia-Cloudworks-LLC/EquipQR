...
- For broad codebase discovery, prefer launching the existing `explore` subagent in read-only mode with a focused prompt that includes the issue number, upstream Problem Record / Service Request summary, suspected surfaces, and the exact return format: impacted files/symbols, likely tests, risks, and no implementation.
- For vendor/API setup procedures, prefer the existing `docs-researcher` subagent to fetch current documentation and return source URLs, dates pulled, setup steps, gotchas, and "could not confirm" gaps for **External Setup Procedures**.
- Do **not** create a custom subagent for normal Change Record drafting unless `explore` and `docs-researcher` prove insufficient in practice. The parent agent remains responsible for Change Record file creation, GitHub posting, Plan-mode handoff, branch creation, commits, PR creation, PR issue linking, and URL capture.

**Triage output**

- Produce a short **triage summary** (in chat) that lists: issue scope, key code paths touched, open risks, the branch name from **Branch & Commit Plan**, and the reusable Change Record file path (`tmp/itil/change-record-issue-<number>.md`) — this summary feeds **Implementation Steps** and **Risk & Impact Analysis**.
...
1. **Pre-Plan Triage then Plan mode for authoring.** Complete **Pre-Plan Triage** above. After triage, you **must** be in Plan mode before drafting the Change Record markdown. Use `SwitchMode` as needed so the sequence is always: triage (Agent mode) → Plan mode → author Change Record → write/update the reusable Change Record file → post that file to the issue → reuse the same file/body for `CreatePlan` → await approval.
...
3. **No product code during Change Record drafting.** Do not modify, create, or delete product files while drafting the Change Record. The only repo-local write allowed before approval is the reusable workflow artifact under `tmp/itil/` required by Rule 5; do not touch tracked product, config, migration, or documentation files during drafting. Plan mode enforces this for authoring; do not work around it. (Read-only triage reads, the reusable `tmp/itil/` Change Record file, and `gh` documentation comments are allowed as defined elsewhere in this skill.)
...
5. **Post the Change Record as a comment on the linked GitHub issue BEFORE the user clicks "build it" — not after.** While in Plan mode, after authoring the Change Record body but BEFORE calling `CreatePlan`, write the Change Record markdown to the reusable repo-local file `tmp/itil/change-record-issue-<number>.md`, then post that file as a comment on the GitHub issue tied to this change (the same issue referenced in **Short Description**). The change authority — the user — should see the Change Record on the issue thread alongside the upstream Service Request / Problem Record, because that is where ITIL audit review actually happens. Skip ONLY when the change is genuinely ad-hoc with no GitHub issue (per the **Inputs** section). The `gh issue comment` call is permitted in Plan mode as part of plan presentation (the Plan-mode prohibition on non-readonly tools is about codebase / config mutations, not the act of documenting the proposed plan on a GitHub issue thread). Capture the emitted comment URL and reference it in your chat message when presenting the plan, so the user can click through to the audit-trail copy. **Preferred Windows helper:** run `.\scripts\itil\Publish-ItilArtifact.ps1 -Issue <number> -ArtifactType ChangeRecord -BodyFile tmp\itil\change-record-issue-<number>.md -Json` — the JSON envelope includes `commentUrl`. Fall back to `gh issue comment` if helper scripts are unavailable or error for reasons unrelated to the artifact. When revising metadata/body mid-flight, update the same markdown file first, then **prefer:** `.\scripts\itil\Update-ItilChangeRecordComment.ps1` (`-CommentUrl` or `-CommentId`, `-BodyFile tmp\itil\change-record-issue-<number>.md`); fallback: `gh api --method PATCH /repos/{owner}/{repo}/issues/comments/{id}`. Never post fragmented "revised" comments that abandon the authoritative thread.
6. **Implementation Steps MUST end with a final step that opens a PR into `preview`, links that PR to the issue, and posts a follow-up on the linked GitHub issue (issue-tied work).** After `git push` on the feature branch, **always** open a PR into `preview` with `gh pr create --base preview --head <branch>` for issue-tied Change Records governed by this skill — **including** when working in the main worktree at `C:\Users\viral\EquipQR`. Do **not** satisfy issue-tied execution by direct `git push origin preview` under this skill; the audit path is branch → push → PR → merge into `preview`. The PR body MUST use a GitHub-recognized issue-linking / closing keyword such as `Resolves #<number>`, `Fixes #<number>`, or `Closes #<number>` so GitHub links the PR to the issue; an issue comment that merely mentions the PR is not sufficient. The follow-up comment MUST include: commit SHA(s); a **mandatory** markdown link to that PR; explicit wording that the issue is **fixed only once the PR merges into `preview`** (if not yet merged, state pending merge); link back to the Change Record comment URL from Mandatory rule 5; AI Verification outcomes; deviations or `None.`. Skip the PR and this follow-up ONLY when the Change Record is ad-hoc with no GitHub issue (use **Branch & Commit Plan** for ad-hoc integration). **Ordering:** The first two items in **Implementation Steps** (and the first two `CreatePlan` todos when a tool emits todos) must always be: (1) post the Change Record on the issue from `tmp/itil/change-record-issue-<number>.md` and capture `commentUrl` — satisfied during Plan mode per Mandatory rule 5 when the comment already exists before approval; (2) create a **new** branch off the latest `origin/preview` per the skeleton below — not branch sync/rebase on an old branch unless the **Resume-only exception** applies.
7. **Plan for cheap implementation.** Write every Change Record so `Composer 2 (fast)` or Cursor Auto can execute it without architectural inference: name exact files, symbols, props, query keys, commands, fixtures, expected states, and stop conditions. **Do not** use vague todos or step text such as "update the logic", "wire up integration", "handle Supabase", "clean up", "review tests", or "fix as needed" — each step must be concrete enough that an implementer needs no design guesses. If the plan cannot be made that explicit because the scope is too broad, split it into smaller Change Records until each slice is independently verifiable by a cheap model. Premium/MAX recommendations are allowed only when the remaining slice is indivisible for correctness or safety; document that reason in **Recommended Execution Model → Constraints surfaced**.
8. **No review / inspect / triage todos after the plan is written.** **Implementation Steps** and any `CreatePlan` todo list must **not** include tasks to review, inspect, audit, investigate, decide, triage, or "figure out" scope — that work belongs only in **Pre-Plan Triage** before the Change Record exists. If a fact is still unknown when drafting, **stop** and finish triage (or shrink scope) before publishing the plan.
9. **Explicit infrastructure and vendor todos.** Whenever implementation requires interaction outside the repo editor — e.g. Supabase (dashboard, CLI, secrets, storage, migrations promotion), Vercel (env, domains, deployment protection), GitHub Actions (secrets, workflow settings), Google Cloud, Resend, 1Password/vault minting, or other vendor consoles — add **numbered Implementation Steps** at the correct dependency order (before any code that assumes the resource exists). Each such step must cross-reference the matching letter in **External Setup Procedures**, name the verification command or UI check that proves success, and state the stop condition. Do not hide infrastructure-only work inside narrative sections without a matching numbered step.
...
1. **(Issue-tied — mandatory first execution step and first `CreatePlan` todo)** Write the complete Change Record markdown to `tmp/itil/change-record-issue-<number>.md`, then post that exact file on the linked GitHub issue before any branch work, infrastructure steps, or product code edits. **Preferred:** `.\scripts\itil\Publish-ItilArtifact.ps1 -Issue <number> -ArtifactType ChangeRecord -BodyFile tmp\itil\change-record-issue-<number>.md -Json` — capture `commentUrl` and log it in chat. **Fallback:** `gh issue comment <number> --repo <owner>/<repo> --body-file tmp\itil\change-record-issue-<number>.md`. Reuse the same file/body for `CreatePlan`; do not retype or rewrite the plan in chat. If the Change Record was **already** posted in Plan mode (Mandatory rule 5) before approval, treat this step as satisfied: confirm the existing `commentUrl` in chat; do **not** post a duplicate thread. **Ad-hoc (no issue):** write `N/A — no GitHub issue; skip posting.`
2. **(Mandatory second step — fresh branch from latest `origin/preview`)** `git fetch origin preview`. Create a **new** implementation branch from `origin/preview`: `git switch -c <branch> origin/preview` using the branch name from **Branch & Commit Plan** ([branching rule](../../rules/branching.mdc)). This ensures the work starts from the codebase version scheduled for release on `preview`. **Do not** reuse an existing local branch for a **new** Change Record; if the name collides, pick a new suffix or branch name and update **Branch & Commit Plan** in the follow-up if the PR head changes. **Resume-only exception:** when the **same** approved Change Record continues on an already-pushed feature branch, `git switch <branch>` then `git rebase origin/preview` (prefer) or merge from `origin/preview`; resolve conflicts; do not proceed until `git status` is clean — state resume rationale in the implementation follow-up. **Ad-hoc:** same `fetch` + `git switch -c <branch> origin/preview` for the branch in **Branch & Commit Plan**.
3. [First product or infrastructure step — name files, functions, conditions, props, tables, policies, or **Section A/B/…** vendor actions with verification]
4. […]
5. […]
N. **(Mandatory final step when this Change Record is tied to a GitHub issue)** After `git push` succeeds on the feature branch, **open a pull request into `preview`** with `gh pr create --base preview --head <branch> --title "<conventional-commit title>"` (use `--body-file` for multi-line PR bodies on PowerShell). **Issue-tied Change Records under this skill always require a PR** — including on the main worktree at `C:\Users\viral\EquipQR`; do **not** integrate via `git push origin preview`. PR body MUST link the PR to the issue with a GitHub-recognized closing keyword such as `Resolves #<number>`, `Fixes #<number>`, or `Closes #<number>`; do not use a weak reference like `Relates to #<number>` when the PR is intended to satisfy the issue. Capture the PR URL. Then post a follow-up comment on the linked issue (same `#<number>` as **Short Description**) using `gh issue comment <number> --repo <owner>/<repo> --body-file <temp-file>`. The follow-up body MUST contain: (a) one-line status like `Implemented in commit <sha-short> on branch <branch-name>`, (b) `Change Record: <change-record-comment-url>` (Mandatory rule 5), (c) **mandatory** `PR: [<title>](<pr-url>)` into `preview`, (d) explicit line that the issue is **fixed only when that PR merges into `preview`** (if not merged, append pending merge), (e) fenced list of AI Verification commands with pass/fail, (f) `**Deviations:**` bullets or `None.`. **Completion gate:** Log in chat all three URLs — Change Record comment, PR to `preview`, implementation follow-up comment. Skip this final step ONLY for ad-hoc Change Records with no GitHub issue.
...
- **Branch:** `<type>/issue-<number>-<kebab-slug>` (off `origin/preview`) — or `<type>/<kebab-slug>` if ad-hoc with no issue. Default for every **new** Change Record: a **new** branch created with `git fetch origin preview` and `git switch -c <branch> origin/preview`. Reusing an older branch is **resume-only** (same approved record, continued work) and must be stated in the implementation follow-up.
- **Change Record file:** `tmp/itil/change-record-issue-<number>.md` for issue-tied work, written once before posting and reused for the GitHub comment, Plan-mode handoff, and metadata PATCHes. For ad-hoc work, use `tmp/itil/change-record-<kebab-slug>.md`.
- **Commits:** Conventional Commits. If tied to an issue, commit body or PR body MUST reference the issue with a GitHub-recognized closing keyword (e.g. `Resolves #<number>`) so GitHub links the PR to the issue and merge into `preview` can close it when appropriate.
- **PR (issue-tied):** **Always** open a PR into `preview` after push with `gh pr create --base preview --head <branch>` — including from the main worktree at `C:\Users\viral\EquipQR`. Do **not** use the direct-push-to-`preview` shortcut for issue-tied work under this skill; repo-wide fast paths in [branching rule](../../rules/branching.mdc) rule 8 do not override this skill's audit requirement. The PR body must include `Resolves #<number>`, `Fixes #<number>`, or `Closes #<number>`; a follow-up issue comment alone is not an acceptable substitute for GitHub's PR-to-issue link.
...
- **Implementation Steps**: Numbered, ordered, **cheap-model executable** (clone/checkout assumptions, files to touch, symbols to edit, migrations order, props, query keys, fixture names, expected UI states, stop conditions, etc.). The **first** step must always be issue posting (or `N/A` ad-hoc); the **second** must always be fresh branch from `origin/preview` (or resume-only rebase/merge per the skeleton). No "update the logic" — name the function, the condition, the table. **No** review/inspect/triage/decision todos (Mandatory rule 8). **No** vague imperatives (Mandatory rule 7). Do not assume the implementer will infer architecture from vague prose. When **External Setup Procedures** has subsections, cross-reference them by letter (e.g. "Complete **Section A** before this step") and mirror each prerequisite with a numbered Implementation Step that includes verification.
...
1. Write the Change Record markdown body to a reusable UTF-8 repo-local file — `tmp\itil\change-record-issue-<number>.md` for issue-tied work, or `tmp\itil\change-record-<kebab-slug>.md` for ad-hoc work. Create the directory first with `New-Item -ItemType Directory -Force tmp\itil` if needed. Do NOT use `--body "..."` with multi-line content on PowerShell because `${{`, heredocs, or odd escapes explode (see `AGENTS.md`). Do not delete this file until the implementation follow-up has been posted or the user abandons the Change Record; it is the source of truth for GitHub posting, Plan-mode handoff, and later metadata PATCHes.
2. **Preferred:** validate headers + emit JSON with `commentUrl`:
   ```powershell
   .\scripts\itil\Publish-ItilArtifact.ps1 `
     -Issue <number> `
     -ArtifactType ChangeRecord `
     -BodyFile tmp\itil\change-record-issue-<number>.md `
     -Json
   ```
3. **Fallback:** `gh issue comment <number> --repo <owner>/<repo> --body-file tmp\itil\change-record-issue-<number>.md`.
...
5. Keep the markdown file in place; do not delete it after posting.
6. Call `CreatePlan` with the same Change Record markdown read from the reusable file so Cursor surfaces the approve/build affordance. When the tool also emits a **todo** list, the **first** todo must be "Post the Change Record on the linked GitHub issue from `tmp/itil/change-record-issue-<number>.md` (capture `commentUrl`)." and the **second** must be "Create a new branch from `origin/preview` (`git fetch` + `git switch -c <branch> origin/preview`)." If the comment was already posted in steps 1–5 above, mark the first todo **completed** immediately and paste the known URL.
7. Wait for the user to authorize build ("approved", "go", etc.).

If revisions land before approval, update the reusable markdown file first, PATCH the authoritative comment (**preferred:** `.\scripts\itil\Update-ItilChangeRecordComment.ps1` with `-CommentUrl`/`-CommentId` and `-BodyFile tmp\itil\change-record-issue-<number>.md`; **fallback:** `gh api repos/<owner>/<repo>/issues/comments/<id> --method PATCH --input <json-file>` housing `{"body": "...full refreshed markdown..."}`), refresh `CreatePlan` from the same file/body, and never orphan a second "revised" comment.
...
2. **Edit the Change Record comment in place to flip the metadata block** (`**Status:** Approved …`). **Preferred:** update the reusable `tmp\itil\change-record-issue-<number>.md` file with the full `# Change Record` markdown + metadata, then invoke `.\scripts\itil\Update-ItilChangeRecordComment.ps1 -CommentUrl <captured-change-record-url> -BodyFile tmp\itil\change-record-issue-<number>.md` (or `-CommentId` if you persisted the REST id earlier). **Fallback:** `gh api repos/<owner>/<repo>/issues/comments/<comment-id> --method PATCH --input <json-file>` wrapping the rebuilt body in `{"body":...}`. Preserve the verbatim Change Record content except for deliberate metadata deltas.
...
4. Execute **Implementation Steps** in order, exactly as written, **except** defer the **last** step (mandatory GitHub follow-up per Mandatory rule 6) until after steps 8–9 below complete — that comment must publish only once `git push` and preview PR publication succeed on issue-tied flows. Step **1** (issue posting) is usually already satisfied from Plan mode; if not, post now from the reusable Change Record file before Step **2**. For Step **2** (branch), prefer `git fetch origin preview` and `git switch -c <branch> origin/preview` for new work; `.\scripts\itil\Start-ItilIssueBranch.ps1` may be used when it matches the **Resume-only exception** (existing branch needs fetch/rebase) — do **not** use a rebase-on-old-branch path when the Change Record calls for a **new** branch. If a revision invalidates execution, update the reusable file and PATCH the authoritative Change Record comment again using `.\scripts\itil\Update-ItilChangeRecordComment.ps1` (fallback `gh api`) instead of spawning new comments.
...
8. **Open a PR into `preview` (issue-tied — mandatory):**
   ...
   .\scripts\itil\New-ItilPreviewPr.ps1 `
     ...
     -Json
   ```
   The PR body file MUST include `Resolves #<number>`, `Fixes #<number>`, or `Closes #<number>` so GitHub links the PR to the issue. The JSON payload echoes `prUrl`. **Fallback:** `gh pr create --base preview --head <branch> --title "<title>" --body-file <pr-body.md>`. Capture the PR URL manually if you fallback. Still **never** push issue-tied work straight to `preview` to dodge review.
...
- Use existing subagents before creating new ones: `explore` for read-only codebase discovery during **Pre-Plan Triage**, `docs-researcher` for fresh vendor/API documentation used by **External Setup Procedures**, `ci-watcher` / `ci-investigator` after a PR exists, and `changelog-version-curator` only for release metadata/version work. Keep branch creation, GitHub posting, PR creation, PR issue linking, and URL capture in the parent agent.
