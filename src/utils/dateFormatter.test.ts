...
describe('dateFormatter', () => {
  ...
  describe('formatDate', () => {
    it('renders different calendar dates for ET vs Sydney', () => {
      expect(formatDate(UTC_CROSS_CALENDAR, settingsNy)).toMatch(/12\/24\/2023/);
      expect(formatDate(UTC_CROSS_CALENDAR, settingsSydney)).toMatch(/12\/25\/2023/);
    });
  ...
  describe('formatTime', () => {
    it('renders local clock differently for ET vs Sydney', () => {
      ...
      expect(formatTime(UTC_CROSS_CALENDAR, settingsSydney)).toMatch(/2:00\s+AM/i);
    });
  });
