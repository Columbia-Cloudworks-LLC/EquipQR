    ...
    <div className={`rounded-lg p-4 space-y-3 ${isFromInventory ? 'bg-info/10 dark:bg-info/15 border border-info/30 dark:border-info/40' : 'bg-muted/50'}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {isFromInventory && (
            <Package className="h-4 w-4 text-info-foreground0" title="From inventory - removing will restore stock" />
          )}
          <span className="text-sm font-medium text-muted-foreground">
            Description {isFromInventory && <span className="text-info-foreground0">(Inventory)</span>}
          </span>
        </div>
        <Button
          type="button"
