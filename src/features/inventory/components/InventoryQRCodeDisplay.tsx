...
const InventoryQRCodeDisplay: React.FC<InventoryQRCodeDisplayProps> = ({ open, onClose, itemId, itemName }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className={`max-w-md ${isMobile ? 'max-h-[calc(100dvh-2rem)] overflow-y-auto p-4' : ''}`}>
        <DialogHeader>
          <DialogTitle>Inventory Item QR Code</DialogTitle>
          <DialogDescription className="sr-only">
            Generate, view, and download QR code for inventory item {itemName || itemId}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className={`${isMobile ? 'space-y-4' : 'space-y-6'}`}>
          ...
          <div className="text-sm text-muted-foreground bg-muted p-3 rounded-lg">
            ...
            <ul className="list-disc list-inside space-y-1 text-xs">
              <li>Print this QR code and attach it to the inventory item or storage location</li>
              <li>Users can scan it with any QR code scanner</li>
              <li>They'll be taken directly to this inventory item's details</li>
              <li>Scans can be used to quickly access item information and transaction history</li>
            </ul>
          </div>
