...
- **Node.js** — must satisfy `engines.node` in the root [`package.json`](../../package.json) (currently **Node 24.x**, matching the Vercel preview/production build image). We recommend the latest **24.x** LTS line for local development. [Download here](https://nodejs.org/)
