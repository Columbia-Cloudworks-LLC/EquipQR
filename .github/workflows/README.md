...
- The `supabase-write` item holds Supabase tooling secrets such as `SUPABASE_ACCESS_TOKEN`, `preview_anon_public_key`, `prod_anon_public_key`, `prod_db_password` (CI: production-release-readiness), and `preview_db_password` (local / preview CLI only unless a future workflow loads it). See `docs/ops/agent-secrets-and-access.md` and `docs/ops/deployment.md`.
