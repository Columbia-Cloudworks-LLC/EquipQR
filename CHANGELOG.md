...
- **Faster Vitest hotspots (#1355)** — Consolidate Privacy/Terms and Reports happy-path renders; sync `lazyPublicPages` / `lazyDashboardPages` stubs in `App.routes`; stub DateTimePicker calendar + ledger date-range picker; lighten EquipmentQRQuickActions PM/photo children so Select/upload use `fireEvent`. Add focused `QRWorkOrderDialog` coverage for real Radix PM Select and photo-picker wiring. Reduces the 2026-07-19 duration-report offenders while keeping #1314 as the umbrella maintainability track.
