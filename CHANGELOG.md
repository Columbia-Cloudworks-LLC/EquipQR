...
- **dev-start/dev-stop startup flow** — `dev-start.bat` now supports `-Force`/`--force` for a full fresh reset (hard stop aligned to `dev-stop -Force`, plus DB reset and type regeneration) before startup, and `--gen-types` no longer short-circuits runtime startup. Reset + type generation now complete as part of one atomic flow that still starts Edge Functions and Vite.
