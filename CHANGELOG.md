...
- **equipqr.info branding aligned with equipqr.app (#1358)** — VitePress theme mirrors Mission Control HSL tokens (primary, neutrals, semantic status colors), uses `appearance: force-dark` (no light toggle), EquipQR wordmark with secondary Docs label, styles the Open App nav CTA as a primary button, allows Plus Jakarta Sans via docs CSP (`style-src` fonts.googleapis.com + `font-src` fonts.gstatic.com), and documents token propagation in `docs/ops/deployment.md`.
