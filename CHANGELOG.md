...
- **dev-start/dev-stop startup flow** — `dev-start.bat` now supports `-Force`/`--force` to run a full hard reset (aligned to `dev-stop -Force`) before startup, and `--gen-types` no longer short-circuits runtime startup. Database reset + type generation now complete as part of one atomic flow that still starts Edge Functions and Vite.
