...
- **Cookie consent banner (#1386)** — First-visit Accept/Reject notice for cookies and browser storage on landing, auth, public QR, and dashboard shells. Persists `equipqr:cookie-consent`; preference storage (including `sidebar:state`) writes only after Accept and is cleared on Reject. Privacy Policy §5 documents the consent key and Accept/Reject behavior; hCaptcha and Maps remain available as non-advertising functional widgets.
