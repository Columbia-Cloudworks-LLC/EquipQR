...
import { defaultSkidSteerChecklist } from '../src/services/preventativeMaintenanceService';
...
async function seedSkidSteerTemplate() {
  try {
    // First, check if the global template already exists
    const { data: existingTemplate } = await supabase
      .from('pm_checklist_templates')
      .select('id')
      .eq('name', 'Skid Steer PM (Default)')
      .eq('organization_id', null)
      .single();

    if (existingTemplate) {
      console.log('✅ Global Skid Steer PM (Default) template already exists');
      return;
    }
    ...
    const sanitizedChecklist = defaultSkidSteerChecklist.map(item => ({
      ...item,
      condition: null,
      notes: ''
    }));
    ...
    if (error) {
      console.error('❌ Error creating global Skid Steer PM (Default) template:', error);
      process.exit(1);
    }

    console.log('✅ Successfully created global Skid Steer PM (Default) template');
