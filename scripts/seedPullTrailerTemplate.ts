...
import { defaultPullTrailerChecklist } from '../src/services/preventativeMaintenanceService';
...
async function seedPullTrailerTemplate() {
  try {
    // First, check if the global template already exists
    const { data: existingTemplate } = await supabase
      .from('pm_checklist_templates')
      .select('id')
      .eq('name', 'Pull Trailer PM (Default)')
      .eq('organization_id', null)
      .single();

    if (existingTemplate) {
      console.log('✅ Global Pull Trailer PM (Default) template already exists');
      return;
    }
    ...
    const sanitizedChecklist = defaultPullTrailerChecklist.map(item => ({
      ...item,
      condition: null,
      notes: ''
    }));
    ...
    if (error) {
      console.error('❌ Error creating global Pull Trailer PM (Default) template:', error);
      process.exit(1);
    }

    console.log('✅ Successfully created global Pull Trailer PM (Default) template');
