...
async function seedCompressorTemplate() {
  try {
    // First, check if the global template already exists
    const { data: existingTemplate } = await supabase
      .from('pm_checklist_templates')
      .select('id')
      .eq('name', 'Compressor PM (Default)')
      .eq('organization_id', null)
      .single();

    if (existingTemplate) {
      console.log('✅ Global Compressor PM (Default) template already exists');
      return;
    }
    ...
    const sanitizedChecklist = defaultCompressorChecklist.map(item => ({
      ...item,
      condition: null,
      notes: ''
    }));
    ...
    if (error) {
      console.error('❌ Error creating global Compressor PM (Default) template:', error);
      process.exit(1);
    }

    console.log('✅ Successfully created global Compressor PM (Default) template');
