-- Migration: Guard equipment-org consistency in is_valid_work_order_assignee
--
-- Security fix: The previous version returned TRUE for org admins/owners
-- before verifying that p_equipment_id belongs to p_organization_id.
-- Because work_orders.equipment_id and work_orders.organization_id are
-- separate FKs (no compound constraint), inconsistent rows could allow
-- cross-tenant assignments if an admin of Org B is validated against
-- equipment belonging to Org A.
--
-- Fix: Verify equipment exists for (p_equipment_id, p_organization_id)
-- before any admin/owner or team-member checks. If the equipment doesn't
-- belong to the specified org, the function returns FALSE immediately.
