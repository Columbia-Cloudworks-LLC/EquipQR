-- Historical work order timeline control (#1080)
-- Adds admin-only RPC to replace backdated operational timelines while preserving
-- real-time audit log timestamps for digitization edits.

-- rpc-authenticated-grant-allowed: replace_historical_work_order_timeline

CREATE OR REPLACE FUNCTION public.historical_timeline_allowed_next_statuses(
...
CREATE OR REPLACE FUNCTION public.synthesize_historical_timeline_events(
...
GRANT EXECUTE ON FUNCTION public.replace_historical_work_order_timeline(uuid, jsonb, boolean) TO authenticated;

COMMENT ON FUNCTION public.replace_historical_work_order_timeline(uuid, jsonb, boolean)
IS 'Admin-only replacement of backdated historical work order timeline events. Writes operational dates to work_order_status_history and syncs derived work order fields without backdating audit_log timestamps. Issue #1080.';

COMMENT ON FUNCTION public.create_historical_work_order_with_pm(
  uuid, uuid, text, text, public.work_order_priority, public.work_order_status, timestamptz,
  text, uuid, uuid, timestamptz, timestamptz, boolean, text, timestamptz, text, jsonb, jsonb
)
IS 'Creates a historical work order and initializes its operational timeline from explicit events or a synthesized valid lifecycle chain. Issue #1080.';
